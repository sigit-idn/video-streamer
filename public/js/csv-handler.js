if (document.querySelector("#uploadCsv")) {
    ...
    function fixOrder () {
        document.querySelectorAll('[name^=start_pos]').forEach((startPos, i) => startPos.name = `start_pos[${i}]`)
        document.querySelectorAll('[name^=end_pos]').forEach((end_pos, i) => end_pos.name = `end_pos[${i}]`)
        document.querySelectorAll('[name^=chapter_name]').forEach((chapter_name, i) => chapter_name.name = `chapter_name[${i}]`)
	document.querySelectorAll('[name^=url]').forEach((url, i) => url.name = `url[${i}]`)
	document.querySelectorAll('#csvTable tbody td:first-child').forEach((iteration, i) => iteration.innerHTML = `${i+1}`)
}
...
function insertRow (element) {
    ...
    if (element) {
        document.querySelector("#csvTable tbody").insertBefore(row, element.parentElement.parentElement.nextElementSibling)
    }
    else {
...
function switchStep() {
	...
	document.querySelector("#stepSwitcher").innerHTML = isContinue ? "Continue" : "Back";
	document.querySelectorAll("[class*=step-]").forEach(step =>
		step.classList.toggle("d-none"))
    document.querySelectorAll("[data-required]").forEach(requiredField => requiredField.setAttribute("required", true))
}
