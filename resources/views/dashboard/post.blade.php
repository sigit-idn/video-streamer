...
@section("dashboard-main")
      ...
      <div class="table-responsive mt-3">
        ...
        <div class="step-2 d-none">
          ...
          <p class="invalid-feedback">{{ $message }}</p>
                    @enderror
                    <button type="button" onclick="removeThumbnail()" class="btn btn-outline-danger d-none mt-1"><i class="bi bi-trash"></i> Remove Uploaded Image</button>
                </div>
          </div>
          <div class="mb-3">
            <label for="title" class="form-label">Title</label>
            <input data-required class="form-control @error("title") is-invalid @enderror" name="title" value="{{ old("title") }}" id="title" placeholder="Insert title">
            @error("title")
          <p class="invalid-feedback">{{ $message }}</p>
            @enderror
          </div>
          <div class="mb-3">
            ...
            <textarea class="form-control @error("tags") is-invalid @enderror" name="tags" value="{{ old("tags") }}" id="tags" placeholder="Insert tags, separated by comma (',')"></textarea>
            @error("tags")
          <p class="invalid-feedback">{{ $message }}</p>
            @enderror
          </div>
          <div class="mb-3">
            ...
            <textarea class="form-control @error("description") is-invalid @enderror" name="description" id="description" placeholder="Insert Description">{{ old("description") }}</textarea>
            @error("description")
          <p class="invalid-feedback">{{ $message }}</p>
            @enderror
          </div>
